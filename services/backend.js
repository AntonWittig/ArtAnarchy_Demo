...
function canvasPostHandler(req) {
    // Verify all requests.
    const payload = verifyAndDecode(req.headers.authorization);
    const { channel_id: channelId, opaque_user_id: opaqueUserId } = payload;
    var imgData = req.body.imgData;
    var image = imagedata_to_image(imgData);

    sendCanvasBroadcast(channelId, image);
}

function canvasGetHandler(req) {
    // Verify all requests.
    const payload = verifyAndDecode(req.headers.authorization);
    const { channel_id: channelId, opaque_user_id: opaqueUserId } = payload;
    sendCanvasBroadcast(channelId, null);
}

function sendCanvasBroadcast(channelId, image) {
